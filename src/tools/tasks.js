...
export function createHandlers(client) {
  return {
    create_task: async ({ targetPersonIds, targetCompanyIds, ...taskData }) => {
    ...
    get_task: async ({ id }) => text("Task:", await client.request(`/rest/tasks/${id}`)),
    list_tasks: async (args = {}) => {
      const { filter, order_by, depth, limit = 20, offset, starting_after, ending_before, search, status, assigneeId, include_deleted = false } = args;
      const clauses = [];
